...
export const TIER_CONFIGS: Record<string, UserTier> = {
  free: {
    name: 'Free Access',
...
function updateInMemoryUsage(userId: string, timestamp: number) {
...
class UserManagement {
   ...
   */
  async getOrCreateUser(email: string): Promise<string | null> {
    if (!supabase) {
   ...
   */
  async createSubscription(userId: string, tier: string): Promise<boolean> {
   ...
   */
  async getCurrentSubscription(userId: string): Promise<{ tier: string; status: string } | null> {
    if (!supabase) {
   ...
   */
  async isAdminAccount(userId: string): Promise<boolean> {
    try {
      // Admin accounts list
   ...
   */
  async canMakeRequest(userId: string): Promise<UsageCheck> {
   ...
   * Log usage
   */
  async logUsage(userId: string, requestType: string, tokensUsed: number = 0, success: boolean = true): Promise<boolean> {
    try {
      const now = new Date();
      const month = now.getMonth() + 1;
      const year = now.getFullYear();
