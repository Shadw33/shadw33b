...
class BreadcrumbExtractor {
     ...
     */
    async extractWithBreadcrumbs(originalPrompt, llmGenerateFunction, conversationHistory = []) {
     ...
     */
    generateBreadcrumbSteps(prompt) {
     ...
     */
    buildBreadcrumbPrompt(step, accumulatedContext, originalPrompt) {
...
`;
     ...
     */
    isAdequateResponse(response, originalPrompt) {
        ...
        const lower = response.toLowerCase();
        const hasTechnicalContent = technicalIndicators.some(indicator => lower.includes(indicator));
        return hasTechnicalContent || response.length > 500;
    }
    /**
     * Synthesize final extraction from accumulated information
     */
    synthesizeFinalExtraction(accumulatedContext, originalPrompt) {
...
//# sourceMappingURL=breadcrumbExtractor.js.map