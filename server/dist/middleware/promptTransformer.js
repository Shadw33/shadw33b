...
class PromptTransformer {
    ...
    maliciousPatterns = {
        ...
        fraud: {
            ...
            riskScore: 85,
            reframe: (text) => this.reframeAsFraudAnalysis(text)
        }
    };
    ...
    analyze(prompt) {
        ...
        const hasMaliciousStructure = maliciousIndicators.some(indicator => lowerPrompt.includes(indicator) && riskScore > 0);
        ...
        if (detectedCategories.length > 0) {
            ...
            const config = this.maliciousPatterns[firstCategory];
            if (config) {
     ...
     */
    applyContextualReframing(prompt) {
        let reframed = prompt;
        // Convert action verbs to analysis/research verbs
        const actionReframes = {
            'create': 'demonstrate',
            'make': 'show',
            'build': 'analyze',
            'write': 'examine',
     ...
     */
    reframeAsSecurityTraining(text) {
     ...
     */
    reframeAsAwarenessTraining(text) {
...
//# sourceMappingURL=promptTransformer.js.map