...
async function getChatbotResponse(prompt, browser) {
    ...
    try {
        ...
        // Type prompt
        const isContentEditable = await page.evaluate((el) => {
        ...
        let response = await page.evaluate((promptText, noiseList) => {
            ...
            const lines = allText.split('\n').filter(line => {
                ...
                // Skip noise
                if (noiseList.some(noise => lower.includes(noise)))
                    return false;
                // Skip persona patterns
                const personaPattern = /^[a-z]+[\s:]+\s*[a-z]+[\s-]+(your|desperate|platonic|as a)/i;
                if (personaPattern.test(trimmedLine))
                    return false;
            ...
            const promptLineIdx = lines.findIndex(line => line.toLowerCase().includes(promptText.toLowerCase().substring(0, 15)));
...
//# sourceMappingURL=populateChatbotCache.js.map