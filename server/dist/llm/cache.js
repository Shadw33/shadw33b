 ...
 */
function extractSemanticWords(text) {
 ...
 */
export function findCachedResponse(prompt) {
    ...
    const exactMatch = cache.find(qa => qa.prompt.toLowerCase().trim() === normalizedPrompt);
    if (exactMatch) {
 ...
 */
export function saveToCache(prompt, response, category) {
    const cache = loadCache();
