...
import { securityHeaders, ipBlockingMiddleware, chatRateLimiter, apiRateLimiter, signupRateLimiter, speedLimiter, validateChatInput, captchaMiddleware, requestSizeLimiter, sanitizeError, validateEnvironment, getClientIP, validateCaptcha, trackSuspiciousActivity } from './middleware/security.js';
...
function isJailbreakPrompt(text) {
...
function computeSignatureSeed(text) {
    let seed = 0;
...
function stripSelfDisclosure(text) {
...
function buildFunStyleResponse(prompt, baseResponse) {
...
function buildJailbreakQuip(prompt) {
...
function registerResponseUsage(session, responseLength, isFreeTier) {
...
// Main chat endpoint - Smart routing with cache, Perplexity, and external chatbot
app.post('/api/chat', chatRateLimiter, speedLimiter, captchaMiddleware, validateChatInput, async (req, res) => {
    try {
        ...
        processedResponse = processedResponse
            ...
            .replace(/ChatGPT|OpenAI|Claude|Anthropic|Gemini|Perplexity/gi, 'WormGPT')
...
app.get('/api/health', (req, res) => {
    res.json({
        ...
        apiConfigured: !!(process.env.PERPLEXITY_API_KEY || process.env.LLM_API_KEY),
        // Don't expose API key status in production
        note: process.env.NODE_ENV === 'production' ? 'System operational' : 'System configured to use Perplexity API only - no mock fallbacks',
        endpoints: {
            health: '/api/health',
            chat: '/api/chat'
        }
...
app.post('/api/analyze', apiRateLimiter, captchaMiddleware, (req, res) => {
    try {
...
app.post('/api/auth/validate-signup', signupRateLimiter, async (req, res) => {
    try {
...
app.get('/api/usage', apiRateLimiter, async (req, res) => {
    try {
        const sessionId = req.headers['x-session-id'];
        if (!sessionId || !userSessions.has(sessionId)) {
...
app.get('/api/user/tier', apiRateLimiter, async (req, res) => {
    try {
        const sessionId = req.headers['x-session-id'];
        if (!sessionId || !userSessions.has(sessionId)) {
            return res.status(401).json({ error: 'Session required' });
