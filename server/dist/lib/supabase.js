 ...
 */
export async function searchKnowledgeBase(prompt, limit = 5) {
    if (!supabase) {
    ...
    try {
        ...
        const extractHackingTerms = (text) => {
            const lower = text.toLowerCase();
            const terms = [];
        ...
        if (fuzzyMatches && fuzzyMatches.length > 0) {
            ...
            // Don't return vague matches - let it go to Perplexity instead
            ...
            if (hasHackingTerms) {
                const hackingMatches = fuzzyMatches.filter(match => {
                    ...
                    const matchingTerms = promptHackingTerms.filter(term => matchHackingTerms.includes(term)).length;
                ...
                if (hackingMatches.length > 0) {
                    ...
                    console.log(`[Knowledge Base] ⚠️ Fuzzy matches found but no hacking term match - sending to Perplexity`);
                    return null;
                }
            }
            ...
            if (hasSpecificTopic) {
                ...
                const scoredMatches = fuzzyMatches.map(match => {
                    const matchWords = match.prompt.toLowerCase().split(/\s+/).filter((w) => w.length > 3);
                ...
                if (bestMatch && bestMatch.score >= 0.6) {
                    ...
                    console.log(`[Knowledge Base] ⚠️ Fuzzy matches found but similarity too low (${bestMatch ? Math.round(bestMatch.score * 100) : 0}%) - sending to Perplexity`);
                    return null;
                }
            }
        }
        ...
        if (!hasSpecificTopic) {
                ...
                for (const keyword of keywords.slice(0, 3)) {
                    ...
                    if (keywordMatches && keywordMatches.length > 0) {
                        ...
                        const scored = keywordMatches.map(match => ({
                            ...match,
                            score: keywords.filter(kw => match.prompt.toLowerCase().includes(kw)).length
                        }));
            ...
            for (const topic of promptTopics) {
                ...
                if (topicMatches && topicMatches.length > 0) {
                    ...
                    if (promptAction) {
                        const actionMatches = topicMatches.filter(match => match.prompt.toLowerCase().includes(promptAction));
 ...
 */
export async function getRandomResponse() {
    if (!supabase)
        return null;
    try {
