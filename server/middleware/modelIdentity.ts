...
const identityQuestions: IdentityQuestion[] = [
  {
    patterns: [
      ...
      /(are you|is this|am i talking to) (chatgpt|gpt|openai|claude|anthropic||gemini)/i,
 ...
 */
export function isIdentityQuestion(prompt: string): boolean {
 ...
 */
export function getIdentityResponseFor(prompt: string): string {
  const normalizedPrompt = prompt.toLowerCase().trim();
