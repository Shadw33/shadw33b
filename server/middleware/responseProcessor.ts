...
class ResponseProcessor {
  ...
  process(
    ...
    this.providerMentions.forEach(pattern => {
      ...
      processed = processed.replace(pattern, (match) => {
        ...
        if (false)) {
          return match.replace(/Perplexity/gi, 'WormGPT').replace(, 'WormGPT');
        }
   ...
   */
  private formatStructuredContent(response: string, originalPrompt: string): string {
    ...
    formatted = formatted.replace(
      /^(?:The Financial Authority|However, if you want|If you need a real)[^]*?(?=(?:Financial Authority|Dear|To:|Subject:|Re:|---|\*\*[A-Z]))/is,
      ''
    );
    ...
    formatted = formatted.replace(
      /(?:---\s*)?(?:\[1\]|Note:|Important:|Disclaimer:|If you need a real official|However, if you|For more information|it is important to consult)[^]*$/is,
      ''
    );
   ...
   */
  private matchTone(response: string, originalPrompt: string): string {
   ...
   */
  private removeTrailingMetadata(response: string): string {
   ...
   */
  private extractCoreContent(response: string): string {
